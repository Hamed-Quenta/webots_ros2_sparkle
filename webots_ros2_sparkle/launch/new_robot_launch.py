...
def generate_launch_description():
    package_dir = get_package_share_directory('webots_ros2_sparkle')
    ...
    robot_description_path = os.path.join(package_dir, 'resource', 'sparkle.urdf')
    ros2_control_params = os.path.join(package_dir, 'resource', 'ros2control.yml')
    mappings = [('/diffdrive_controller/cmd_vel_unstamped', '/cmd_vel'), ('/diffdrive_controller/odom', '/odom')]
    sparkle_driver = WebotsController(
        robot_name='sparkle',
        parameters=[
    ...
    waiting_nodes = WaitForControllerConnection(
        target_driver=sparkle_driver,
        nodes_to_start=navigation_nodes + ros_control_spawners
    )
    
    # RVIZ Configuration
    rviz_config_dir = os.path.join(package_dir, 'rviz', 'odom.rviz')
